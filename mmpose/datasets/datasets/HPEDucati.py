...
class HPEDucati(Kpt2dSviewRgbImgTopDownDataset):
	...
	def _get_db(self):
		...
		for idx, ann in enumerate(anns):
			# get image path
			image_file = anns['images'][idx]['file_name']
			# get bbox
			bbox = anns['annotations'][idx]['bbox']
			center, scale = self._xywh2cs(*bbox)
			# get keypoints
			keypoints = np.array(
				anns['annotations'][idx]['keypoints'], dtype=np.float32).reshape(-1, 3)
